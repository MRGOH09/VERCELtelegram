...
export async function personalMorningReportsWithBranch(forDate) {
  ...
  if (personalResults.messages && personalResults.messages.length > 0) {
    ...
    const { data: userBranches } = await supabase
      .from('user_profile')
      .select('chat_id,users(branch_code)')
      .not('chat_id', 'is', null)
    
    const userBranchMap = new Map()
    console.log(`[personalMorningReportsWithBranch] 查询到的用户分行数据:`, userBranches?.map(u => ({ chat_id: u.chat_id, branch_code: u.users?.branch_code })))
    
    userBranches?.forEach(u => {
      if (u.users?.branch_code) {
        userBranchMap.set(u.chat_id, u.users.branch_code)
        console.log(`[personalMorningReportsWithBranch] 映射用户 ${u.chat_id} -> 分行 ${u.users.branch_code}`)
      }
    })
