...
export async function computeLeaderboards(forDate) {
  ...
  // 计算分行统计 - 联查user_profile和users表
  const { data: userProfiles } = await supabase
    .from('user_profile')
    .select('user_id,current_streak,display_name,users!inner(branch_code)')
    .not('user_id', 'is', null)
  ...
  // 统计各分行情况
  userProfiles?.forEach(p => {
    if (!p.users?.branch_code) return
    
    const branchCode = p.users.branch_code
