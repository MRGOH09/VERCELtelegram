...
export async function computeLeaderboards(forDate) {
  ...
  usersData?.forEach(u => {
    if (u.branch_code) {
      userToBranch.set(u.id, u.branch_code)
    }
  })
  
  console.log(`[computeLeaderboards] 查询到 ${userProfiles?.length || 0} 个用户档案，${usersData?.length || 0} 个用户有分行设置`)
  
  console.log(`[computeLeaderboards] 建立了 ${userToBranch.size} 个用户分行映射`)
  ...
  userProfiles?.forEach(p => {
    const branchCode = userToBranch.get(p.user_id)
    ...
    if (userStreak > stats.max_streak) {
      ...
      stats.max_streak_user = p.display_name || `用户${p.user_id}`
    }
