...
export async function personalMorningReports(forDate, templateFn) {
  ...
  const { data: lastMonthSummaries } = await supabase
    .from('daily_summary')
    .select('user_id,sum_a,sum_b,sum_c')
  ...
  // 上月数据
  for (const s of lastMonthSummaries || []) {
    if (!lastMonthMap.has(s.user_id)) lastMonthMap.set(s.user_id, { a: 0, b: 0, c: 0 })
    const m = lastMonthMap.get(s.user_id)
    m.a += Number(s.sum_a || 0)
    m.b += Number(s.sum_b || 0)
    m.c += Number(s.sum_c || 0)
  }
  ...
  const users = (profs || []).filter(p => p.chat_id).map(p => {
    ...
    const lastMonthSum = lastMonthMap.get(p.user_id) || { a: 0, b: 0, c: 0 }
    ...
    // 计算上月对比
    const lastMonthTotal = lastMonthSum.a + lastMonthSum.b + lastMonthSum.c
