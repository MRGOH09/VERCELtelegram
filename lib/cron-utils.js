...
export async function personalMorningReportsWithBranch(forDate) {
  ...
  console.log(`[personalMorningReportsWithBranch] 获取到 ${branchRankings.size} 个分行排行榜`)
  
  // 2. 使用原来personalMorningReports的逻辑获取个人报告（注意这里会发送消息）
  ...
  console.log(`[personalMorningReportsWithBranch] personalMorningReports 返回: 发送 ${personalResults.sent}, 失败 ${personalResults.failed}, 消息数 ${personalResults.messages?.length || 0}`)
  ...
  console.log(`[personalMorningReportsWithBranch] 找到 ${userBranches?.length || 0} 个有分行的用户`)
  ...
  if (personalResults.messages && personalResults.messages.length > 0) {
    ...
    // 发送分行排行榜消息
    const branchSendResults = await sendBatchMessages(branchMessages)
    console.log(`[personalMorningReportsWithBranch] 分行排行榜发送结果: 成功 ${branchSendResults.sent}, 失败 ${branchSendResults.failed}`)
    
    return {
      sent: personalResults.sent + branchSendResults.sent,
      failed: personalResults.failed + branchSendResults.failed,
      totalTime: (personalResults.totalTime || 0) + (branchSendResults.totalTime || 0),
      rate: personalResults.rate || 0,
      messages: personalResults.messages
    }
  }
  
  console.log(`[personalMorningReportsWithBranch] 没有消息需要处理，直接返回personalResults`)
