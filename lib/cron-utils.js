...
export async function pushBranchLeaderboards(forDate, templateFn) {
  ...
  // 计算平均分、排序并设置排名
  const branchArray = Array.from(branchStatsMap.values())
    .map(branch => ({
      ...branch,
      ...
      participation_rate: branch.total_members > 0
        ? Math.round((branch.active_members / branch.total_members) * 100)
        : 0,
      done: branch.active_members,
      total: branch.total_members
    }))
  ...
  console.log(`[分行推送] 实时计算完成: ${branchArray.length} 个分行`)
  
  // 获取分行负责人
  const { data: leads } = await supabase.from('branch_leads').select('branch_code,leader_chat_ids')
  const branchMap = new Map(branchArray.map(b => [b.branch_code, b]))
  
  const messages = []
  for (const row of leads || []) {
    const stat = branchMap.get(row.branch_code)
    if (!stat) continue
    
    const text = templateFn(row.branch_code, { 
      ...stat, 
      avg_7day_score: stat.avg_score // 使用当前平均分作为7日均分
    })
    
    for (const cid of row.leader_chat_ids || []) {
      messages.push({ chat_id: cid, text })
    }
  }
...
export async function personalMorningReports(forDate, templateFn) {
  ...
  const messages = resolvedUsers.map(u => ({ 
    chat_id: u.chat_id, 
    text: templateFn(u.income, u.a, u.b, u.c, u.ra, u.rb, u.rc, u.completion, u.progress, u.budget_a, u.budget_status_a, u.recordMetrics?.totalRecords || 0, u.recordMetrics?.recordDays || 0, u.recordMetrics?.avgRecordsPerDay || '0.0') 
  }))
...
export async function personalMorningReportsWithBranch(forDate) {
  ...
  const personalResults = await personalMorningReports(forDate, (income, a, b, c, ra, rb, rc, completion, progress, budget_a, budget_status_a, total_records, record_days, avg_records_per_day) => {
    const baseMessage = formatTemplate(zh.cron.morning_rank, { 
      income, a, b, c, ra, rb, rc, completion, progress, budget_a, budget_status_a, total_records, record_days, avg_records_per_day 
    })
...
export async function getBranchRankingsData(forDate) {
  ...
  for (const [branchCode, stat] of map.entries()) {
    ...
    const branchText = formatTemplate(zh.cron.branch_lead, {
      code: branchCode,
      rank: stat.rank || 1,
      avg_score: stat.avg_score || stat.rate || 0,  // 兼容旧数据
      participation_rate: stat.participation_rate || Math.round((stat.done || 0) / (stat.total || 1) * 100),
      done: stat.done || 0,
      total: stat.total || 0,
      total_score: stat.total_score || 0,
      avg_7day_score: avg7 || stat.avg_score || stat.rate || 0
    })
