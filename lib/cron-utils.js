...
export async function personalMorningReportsWithBranch(forDate) {
  console.log('[personalMorningReportsWithBranch] 开始生成一条完整的理财+分行消息...')
  ...
  // 2. 完全复制personalMorningReports的逻辑，但不调用sendBatchMessages
  const { data: recs } = await supabase
    .from('records')
    .select('user_id')
    .eq('ymd', ymd)
  ...
  console.log(`[personalMorningReportsWithBranch] 原始用户查询结果: ${profs?.length || 0} 个用户, 错误:`, profsError)
  if (profs && profs.length > 0) {
    console.log(`[personalMorningReportsWithBranch] 用户示例:`, profs[0])
    console.log(`[personalMorningReportsWithBranch] 所有用户的chat_id:`, profs.map(p => ({ user_id: p.user_id, chat_id: p.chat_id })))
  }
  
  const byChat = (profs || []).filter(p => p.chat_id)
  console.log(`[personalMorningReportsWithBranch] 过滤后有chat_id的用户: ${byChat.length} 个`)
  
  if (byChat.length === 0) {
    console.log('[personalMorningReportsWithBranch] 没有用户，返回空结果')
    return { sent: 0, failed: 0, totalTime: 0, rate: 0 }
  }
  ...
  try {
    await batchEnsureMonthlyBudgets(budgetRequests)
  } catch (error) {
    console.error(`[personalMorningReportsWithBranch] 批量创建预算失败，继续执行:`, error)
  }

  // 生成每个用户的完整消息（理财+分行）
  const users = byChat.map(async (p) => {
    try {
      console.log(`[personalMorningReportsWithBranch] 开始处理用户 ${p.user_id}, chat_id: ${p.chat_id}`)
      
      // 复制原来personalMorningReports的用户数据计算逻辑
      ...
      if (recordsError) {
        console.error(`[personalMorningReportsWithBranch] 查询用户 ${p.user_id} 记录失败:`, recordsError)
      }
      ...
      if (profileError) {
        console.error(`[personalMorningReportsWithBranch] 查询用户 ${p.user_id} 资料失败:`, profileError)
      }

      const monthlyIncome = Number(profile?.monthly_income || 0)
      
      // 允许收入为0的用户也收到消息，只是显示为0
      console.log(`[personalMorningReportsWithBranch] 用户 ${p.user_id} 月收入: ${monthlyIncome}`)
      
      // 按分组统计本月数据
      const groupStats = { A: { total: 0 }, B: { total: 0 }, C: { total: 0 } }
      const recordDates = new Set()
      ...
      records?.forEach(record => {
        ...
        if (record.ymd) recordDates.add(record.ymd)
      })
      
      const recordMetrics = {
        totalRecords: records?.length || 0,
        recordDays: recordDates.size,
        avgRecordsPerDay: recordDates.size > 0 ? (records.length / recordDates.size).toFixed(1) : '0.0'
      }

      // 计算各项数据
      const travelMonthlyNum = Number(profile?.travel_budget_annual || 0) / 12
      const a = groupStats.A.total
      const b = groupStats.B.total + travelMonthlyNum
      ...
      const completion = monthlyIncome > 0 ? Math.min(100, ((a + b + c) / monthlyIncome * 100)).toFixed(1) : '0.0'

      // 生成理财报告部分
      const personalMessage = formatTemplate(zh.cron.morning_rank, { 
        income: monthlyIncome,
        a: a.toFixed(2), b: b.toFixed(2), c: c.toFixed(2),
        ra, rb, rc, completion,
        progress: '+0%',
        streak: Number(p.current_streak || 0),
        budget_a: Number(profile?.a_pct || 0),
        budget_status_a: `还剩 ${Number(profile?.a_pct || 0).toFixed(1)}%`,
        total_records: recordMetrics.totalRecords,
        record_days: recordMetrics.recordDays,
        avg_records_per_day: recordMetrics.avgRecordsPerDay
      })

      // 如果在9月挑战期间，添加挑战消息
      let finalPersonalMessage = personalMessage
      if (isInSeptemberChallenge(forDate)) {
        finalPersonalMessage = addChallengeMessageToMorningRank(personalMessage, forDate)
      }

      // 添加分行排行榜部分 - 需要单独查询分行信息
      let userBranch = null
      try {
        const { data: branchData } = await supabase
          ...
          .eq('user_id', p.user_id)
          .single()
        userBranch = branchData?.branch_code
      } catch (e) {
        console.log(`[personalMorningReportsWithBranch] 查询用户 ${p.user_id} 分行失败:`, e.message)
      }
      let branchMessage = '\n\n📊 分行排行榜:\n'
      
      if (userBranch) {
        const branchRanking = branchRankings.get(userBranch)
        branchMessage += `🏢 您的分行: ${userBranch}\n`
        if (branchRanking) {
          branchMessage += `${branchRanking}`
        } else {
          branchMessage += `暂无排行榜数据`
        }
      } else {
        branchMessage += `您还没有设置分行代码`
      }

      // 返回完整合并消息
      return {
        chat_id: p.chat_id,
        text: finalPersonalMessage + branchMessage
      }
    } catch (error) {
      console.error(`[personalMorningReportsWithBranch] 处理用户 ${p.user_id} 失败:`, error)
      return null
    }
  })

  const resolvedUsers = (await Promise.all(users)).filter(u => u !== null)
  console.log(`[personalMorningReportsWithBranch] 生成了 ${resolvedUsers.length} 条完整消息`)
  
  if (resolvedUsers.length > 0) {
    console.log(`[personalMorningReportsWithBranch] 消息示例:`, resolvedUsers[0]?.text?.substring(0, 200))
  }

  // 发送合并消息
  if (resolvedUsers.length > 0) {
    console.log(`[personalMorningReportsWithBranch] 开始发送 ${resolvedUsers.length} 条消息...`)
    const sendResult = await sendBatchMessages(resolvedUsers)
    console.log(`[personalMorningReportsWithBranch] 发送完成，结果:`, sendResult)
    return sendResult
  }
  
  console.log(`[personalMorningReportsWithBranch] 没有消息需要发送`)
  return { sent: 0, failed: 0, totalTime: 0, rate: 0 }
}
