  ...
  const users = (profs || []).filter(p => p.chat_id).map(async (p) => {
    try {
      ...
      const { data: budget } = await supabase
        ...
        .select('income,a_pct,b_pct,epf_amount')
        .eq('user_id', p.user_id)
        .eq('yyyymm', yyyyMM)
        .single()
      ...
      // 计算数据
      const income = Number(budget?.income || userData?.monthly_income || 0)
      const aPct = Number(budget?.a_pct || userData?.a_pct || 0)
      const bPct = Number(budget?.b_pct || userData?.b_pct || 0)
      const cPct = Math.max(0, 100 - aPct - bPct)

      // 汇总本月数据
      let a = 0, b = 0, c = 0
      for (const s of monthSummary || []) {
        a += Number(s.sum_a || 0)
        b += Number(s.sum_b || 0)
        c += Number(s.sum_c || 0)
      }
      ...
      // 汇总上月数据
      let lastMonthA = 0, lastMonthB = 0, lastMonthC = 0
      for (const s of lastMonthSummary || []) {
        lastMonthA += Number(s.sum_a || 0)
        lastMonthB += Number(s.sum_b || 0)
        lastMonthC += Number(s.sum_c || 0)
      }
      ...
      // 计算上月对比
      const lastMonthTotal = lastMonthA + lastMonthB + lastMonthC
