...
export async function personalMorningReportsWithBranch(forDate) {
  console.log('[personalMorningReportsWithBranch] 开始生成合并的理财报告+分行排行榜...')
  ...
  // 2. 批量获取所有用户资料（和原personalMorningReports完全一致）
  const { data: profs, error: profsError } = await supabase
    .from('user_profile')
    .select('user_id,chat_id,display_name,current_streak,last_record,monthly_income,a_pct,branch_code,travel_budget_annual')
  
  console.log(`[personalMorningReportsWithBranch] 查询用户资料: ${profs?.length || 0} 个结果, 错误:`, profsError)
  
  const byChat = (profs || []).filter(p => p.chat_id)
  console.log(`[personalMorningReportsWithBranch] 找到 ${byChat.length} 个用户，原始数据示例:`, byChat[0] || '无用户')
  
  // 确保所有活跃用户都有当月预算记录（和原函数一致）
  if (byChat.length > 0) {
    const budgetRequests = byChat.map(p => ({ 
      userId: p.user_id, 
      yyyymm: yyyyMM 
  ...
  console.log(`[personalMorningReportsWithBranch] 查询预算快照: ${budgets?.length || 0} 个结果, 错误:`, budgetsError)
  
  const budgetMap = new Map()
  budgets?.forEach(b => budgetMap.set(b.user_id, b))
  
  // 4. 批量获取所有用户的当月记录
  const { data: records, error: recordsError } = await supabase
    .from('records')
    .select('user_id,amount,category_group,ymd')
    .gte('ymd', `${yyyyMM}-01`)
    .lte('ymd', ymd)
    .eq('is_voided', false)
  
  console.log(`[personalMorningReportsWithBranch] 查询用户记录: ${records?.length || 0} 个结果, 错误:`, recordsError)
  ...
  for (const profile of byChat) {
    const userId = profile.user_id
    const chatId = profile.chat_id
    const userBranch = profile.branch_code
    
    console.log(`[personalMorningReportsWithBranch] 处理用户 ${userId}, chatId: ${chatId}, 分行: ${userBranch}`)
    
    // 获取用户数据
    const budget = budgetMap.get(userId)
    const monthlyIncome = Number(budget?.income || profile.monthly_income || 0)
    const userRecords = userRecordsMap.get(userId) || []
    
    console.log(`[personalMorningReportsWithBranch] 用户 ${userId} - 预算收入: ${budget?.income}, 资料收入: ${profile.monthly_income}, 最终收入: ${monthlyIncome}, 记录数: ${userRecords.length}`)
    
    if (monthlyIncome <= 0) {
      console.log(`[personalMorningReportsWithBranch] ⚠️ 用户 ${userId} 月收入为0或无效，跳过`)
      continue
    }
    ...
    const travelMonthly = Math.round((profile.travel_budget_annual || 0) / 12 * 100) / 100
    const bTotal = Math.round((groupStats.B.total + travelMonthly) * 100) / 100
    const cTotal = groupStats.C.total
    
    const ra = monthlyIncome > 0 ? (aTotal / monthlyIncome * 100).toFixed(1) : '0.0'
    const rb = monthlyIncome > 0 ? (bTotal / monthlyIncome * 100).toFixed(1) : '0.0'
    const rc = monthlyIncome > 0 ? (cTotal / monthlyIncome * 100).toFixed(1) : '0.0'
    
    const completion = monthlyIncome > 0 ? Math.min(100, ((aTotal + bTotal + cTotal) / monthlyIncome * 100)).toFixed(1) : '0.0'
    const streak = Number(profile.current_streak || 0)
    const budget_a = Number(profile.a_pct || 0)
    
    // 生成个人理财报告部分
    const personalMessage = formatTemplate(zh.cron.morning_rank, {
      income: monthlyIncome,
      a: aTotal.toFixed(2), b: bTotal.toFixed(2), c: cTotal.toFixed(2),
      ra, rb, rc, completion,
      progress: '+0%',
      streak,
      budget_a,
      budget_status_a: `还剩 ${budget_a.toFixed(1)}%`,
      total_records: userRecords.length,
      record_days: recordDates.size,
      avg_records_per_day: recordDates.size > 0 ? (userRecords.length / recordDates.size).toFixed(1) : '0.0'
    })
    ...
    if (userBranch) {
      ...
      if (branchRanking) {
        ...
        branchMessage += `❌ 分行 ${userBranch} 暂无排行榜数据\n📋 系统会在首次记账后生成排行榜`
      }
    } else {
    ...
    // 合并完整消息
    const fullMessage = finalPersonalMessage + branchMessage
    
    console.log(`[personalMorningReportsWithBranch] 用户 ${userId} 消息长度: ${fullMessage.length}`)
  ...
  console.log(`[personalMorningReportsWithBranch] 生成了 ${messages.length} 个合并消息`)
