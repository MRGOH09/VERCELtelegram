...
export async function calculateDailyScore(userId, date, recordType = 'record') {
  ...
  const streakScore = 0  // 取消连续分
  const bonusScore = 0   // 取消奖励分
  ...
  // 4. 更新 user_profile 的最后记录时间（不再更新连续天数）
  try {
    await supabase
      .from('user_profile')
      .update({ 
    ...
    console.log(`[积分计算] 已更新最后记录时间`)
  } catch (syncError) {
    console.error('[积分计算] 同步 user_profile 失败 (不影响积分):', syncError)
  }
  
  console.log(`[积分计算] 积分保存成功: ${savedScore.total_score}分`)
