...
### **核心问题**: API调用不一致导致功能无法正常工作

#### **已识别的问题模式**:
1. **批量记录无法保存**: 使用外部API调用而非直接写入Supabase
2. **排行榜无法显示**: 使用JWT认证而非Supabase统一认证
3. **数据显示异常**: API响应格式不匹配前端期望

#### **统一解决方案**: **PWA原生API优先策略**

##### **第一步: 检查PWA data API是否支持该功能**
```javascript
// 检查 /pages/api/pwa/data.js 中的 switch 语句
case 'target-action': 
  return await targetFunction(userId, params, res)
```

##### **第二步: 如不支持，在PWA data API中添加该功能**
```javascript
// 1. 添加case到switch语句
case 'new-function':
  return await newFunction(userId, params, res)

// 2. 实现对应的函数
async function newFunction(userId, params, res) {
  // 直接使用supabase操作数据库
  // 使用统一的认证和错误处理
}
```

##### **第三步: 在PWAClient中添加对应方法**
```javascript
// 在lib/api.js中添加
async getNewFunction() {
  return this.cachedCall('data', 'new-function', {}, cacheTime)
}
```

##### **第四步: 前端页面使用PWAClient调用**
```javascript
import PWAClient from '../lib/api'

// 替换原有的fetch调用
const result = await PWAClient.getNewFunction()
```

#### **已成功修复的功能**:
- ✅ **批量记录**: 从外部API改为PWA data API的batch-add-records
- ✅ **个性化建议移除**: 改为固定33%显示
- ✅ **批量记录验证**: 修复分类字段验证逻辑
- ✅ **成功提示**: 修复显示实际成功记录数
- ✅ **排行榜显示**: 集成到PWA data API的leaderboard action

#### **修复效果**:
- 🚀 **统一认证**: 全部使用Supabase认证，无需JWT管理
- 📊 **统一缓存**: 智能缓存提升性能
- 🔒 **统一错误处理**: 一致的错误响应格式
- 🎯 **代码复用**: 减少重复API端点维护
