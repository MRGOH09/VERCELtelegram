...
function DataExportPanel() {
  const [exportType, setExportType] = useState('branch-summary')
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = filenameMatch ? filenameMatch[1].replace(/['"]/g, '') : 
        `LEARNER_CLUB_${exportType}_${exportFormat}_${new Date().toISOString().slice(0, 10)}`
  ...
  const exportTypes = [
    { value: 'branch-summary', label: '分院汇总报告', description: '各分院积分排名、成员统计' },
    { value: 'user-details', label: '用户详情报告', description: '用户积分历史、活跃度分析' },
    { value: 'score-analysis', label: '积分分析报告', description: '积分趋势、统计图表' },
    { value: 'comprehensive', label: '综合数据报告', description: '完整的系统数据报告' }
  ]

  const exportFormats = [
    { value: 'pdf', label: 'PDF报告', icon: '📄', description: '专业格式，适合打印和分享' },
    { value: 'excel', label: 'Excel表格', icon: '📊', description: '可编辑数据，适合分析' },
    { value: 'csv', label: 'CSV数据', icon: '📋', description: '纯数据格式，便于导入其他系统' }
  ]
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">导出系统数据为不同格式的报告</p>
          </div>
        </div>
      </div>
      ...
      {exportFormat === 'pdf' && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-medium text-blue-800 mb-2">📋 专业报告布局</h4>
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• LEARNER CLUB品牌封面</li>
                <li>• 清晰的章节分割</li>
                <li>• 页脚页码和品牌标识</li>
                <li>• 中文字体支持</li>
              </ul>
            ...
            <div>
              <h4 className="font-medium text-blue-800 mb-2">📊 数据可视化</h4>
              <ul className="text-sm text-blue-700 space-y-1">
                <li>• 格式化表格展示</li>
                <li>• 关键数据高亮显示</li>
                <li>• 统计摘要和汇总</li>
          ...
          <div className="mt-4 p-3 bg-white rounded border border-blue-200">
            <div className="text-sm text-blue-800">
              <strong>当前选择：</strong> {exportTypes.find(t => t.value === exportType)?.label} - PDF格式
            </div>
            <div className="text-xs text-blue-600 mt-1">
              将生成专业的PDF报告，包含完整的数据分析和可视化图表
            </div>
          </div>
        </div>
      )}
