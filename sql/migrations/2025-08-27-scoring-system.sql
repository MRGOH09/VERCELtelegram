...
create index if not exists idx_user_daily_scores_total_score on user_daily_scores(ymd, total_score desc);

-- 添加注释
comment on table user_daily_scores is '用户每日积分记录表';
comment on table branch_scores_daily is '分行每日积分统计表';
comment on table score_milestones is '积分里程碑配置表';

comment on column user_daily_scores.base_score is '基础分：记录或打卡获得1分';
comment on column user_daily_scores.streak_score is '连续分：连续记录获得1分';
comment on column user_daily_scores.bonus_score is '奖励分：达成里程碑获得奖励';
comment on column user_daily_scores.current_streak is '当前连续记录天数';
comment on column user_daily_scores.record_type is '记录类型：record=开销记录, checkin=纯打卡';
comment on column user_daily_scores.bonus_details is '奖励明细JSON：记录获得的具体奖励';

comment on column branch_scores_daily.avg_score is '分行平均积分：总积分/总人数';
comment on column branch_scores_daily.branch_rank is '分行排名：按平均积分降序排名';

-- 权限设置(如果需要)
-- grant select, insert, update on user_daily_scores to authenticated;
-- grant select, insert, update on branch_scores_daily to authenticated;
-- grant select on score_milestones to authenticated;